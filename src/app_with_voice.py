...
<style>
    ...
    .logo-container { display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 2rem; background-color: #161b22; border-radius: 12px; margin-bottom: 2rem; border: 1px solid #30363d; }
    .logo-text { font-size: 2.5rem; font-weight: 800; background: linear-gradient(90deg, #4da6ff, #00ffff); -webkit-background-clip: text; -webkit-text-fill-color: transparent; margin: 0; letter-spacing: -1px; }
    .tagline { color: #8b949e; font-size: 0.9rem; margin-top: 0.5rem; text-transform: uppercase; letter-spacing: 2px; font-weight: 600; }
    .stTextInput > div > div > input { background-color: #0d1117; color: #c9d1d9; border: 1px solid #30363d; }
    .stTabs [data-baseweb="tab-list"] { background-color: #0e1117; }
    .stTabs [data-baseweb="tab"] { color: #8b949e; }
    .stTabs [aria-selected="true"] { color: #58a6ff !important; border-bottom-color: #58a6ff !important; }
    div.stButton > button:first-child { background-color: #238636; color: white; border: 1px solid rgba(240, 246, 252, 0.1); padding: 0.5rem 1rem; border-radius: 6px; font-weight: 600; width: 100%; transition: 0.2s; }
    div.stButton > button:first-child:hover { background-color: #2ea043; border-color: #8b949e; }
    div.logout-btn > button:first-child { background-color: #da3633 !important; border: 1px solid #da3633 !important; }
    div.logout-btn > button:first-child:hover { background-color: #b62324 !important; }
    .stChatMessage { background-color: #161b22; border: 1px solid #30363d; }
</style>
""", unsafe_allow_html=True)

# ... [YOUR STATE INITIALIZATION REMAINS UNCHANGED] ...
...
# ... [YOUR ASYNC HELPER & AUTH FUNCTIONS REMAIN UNCHANGED] ...
...
if not st.session_state.cust_id:
    # ... [YOUR LOGIN/SIGNUP UI REMAINS UNCHANGED] ...
    ...
    with st.sidebar:
        ...
        # This creates a microphone button. When finished speaking, it returns text.
    ...
    # --- UNIFIED INPUT LOGIC (TEXT OR VOICE) ---
    chat_prompt = st.chat_input("Type your banking query here...")

    # We accept input from EITHER the chat box OR the microphone
    final_input = chat_prompt or voice_text
    ...
    if final_input:
        ...
        # If voice was used, we manually display the user message immediately
        if voice_text:
            with st.chat_message("user"):
                st.markdown(final_input)
        # (If chat_prompt was used, st.chat_input handles the display automatically)

        # 2. Get Agent Response
        with st.chat_message("assistant"):
            with st.spinner("Processing..."):
                response_text = run_async(get_agent_response(final_input))
        ...
        # 4. Rerun if voice was used (to reset the button state)
        if voice_text:
            st.rerun()
